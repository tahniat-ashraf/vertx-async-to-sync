...
public class ProducerApiVerticle extends AbstractVerticle {
  ...
  public void start(Promise<Void> promise) throws Exception {
    ...
    router.post("/addBulkPosts").handler(routingContext -> requestHandler(routingContext, EventBusAddress.BULK_ADD_POSTS)).failureHandler(exceptionHandler);
  ...
  private void requestHandler(RoutingContext routingContext, EventBusAddress eventBusAddress) {
    ...
    logRequest(eventBusAddress, request);
  ...
  private void logRequest(EventBusAddress eventBusAddress, JsonObject request) {
    if(request !=null) {
      LOG.info("requestHandler :: request :: " + request.encode() + "; eventBusAddress :: " + eventBusAddress.name());
    }else{
      LOG.info("requestHandler :: eventBusAddress :: " + eventBusAddress.name());
    }
  }
